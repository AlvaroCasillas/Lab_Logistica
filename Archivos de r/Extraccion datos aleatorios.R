...
length(unique(ecobici_semana$Fecha_Arribo))
intervalo1 <- ecobici_semana %>% filter(Hora_Arribo > as.hms("05:00:00") & Hora_Arribo <= as.hms("13:00:00"))
intervalo2 <- ecobici_semana %>% filter(Hora_Arribo > as.hms("13:00:00") & Hora_Arribo <= as.hms("17:30:00"))
intervalo3 <- ecobici_semana %>% filter(Hora_Arribo > as.hms("17:30:00") & Hora_Arribo <= as.hms("23:59:59"))
viernes <- ecobici_viernes %>% filter(Hora_Arribo >= as.hms("00:00:00") & Hora_Arribo <= as.hms("00:30:00") ) 
intervalo3 <- rbind2(intervalo3, viernes)
ecobici_semana <- rbind2(ecobici_semana,viernes)

atipicos1 <- intervalo1 %>% filter(Tiempo_Viaje  > 570)
atipicos2 <- intervalo2 %>% filter(Tiempo_Viaje  > 570)
atipicos3 <- intervalo3 %>% filter(Tiempo_Viaje  > 570)
atipicostot <- ecobici_semana %>% filter(Tiempo_Viaje > 570)


intervalo1 <- intervalo1 %>% filter(Tiempo_Viaje  <= 570)
intervalo2 <- intervalo2 %>% filter(Tiempo_Viaje  <= 570)
intervalo3 <- intervalo3 %>% filter(Tiempo_Viaje  <= 570)
ecobici_semana <- ecobici_semana %>% filter(Tiempo_Viaje <= 570)

seleccion_random1 <- intervalo1[sample(nrow(intervalo1), 200), ]
datos_int1 <- seleccion_random1$Tiempo_Viaje
write.csv(datos_int1, file = "Datos intervalo 1", row.names = FALSE, col.names = FALSE)


seleccion_random2 <- intervalo2[sample(nrow(intervalo2), 200), ]
datos_int2 <- seleccion_random2$Tiempo_Viaje
write.csv(datos_int2, file = "Datos intervalo 2", row.names = FALSE, col.names = FALSE)


seleccion_random3 <- intervalo3[sample(nrow(intervalo3), 200), ]
datos_int3 <- seleccion_random3$Tiempo_Viaje
write.csv(datos_int3, file = "Datos intervalo 3", row.names = FALSE, col.names = FALSE)


seleccion_random_tot <- ecobici_semana[sample(nrow(ecobici_semana), 200), ]
datos_tot <- seleccion_random_tot$Tiempo_Viaje
write.csv(datos_tot, file = "Datos total", row.names = FALSE, col.names = FALSE)

media_int1 <- mean(intervalo1$Tiempo_Viaje)
media_int2 <- mean(intervalo2$Tiempo_Viaje)
media_int3 <- mean(intervalo3$Tiempo_Viaje)
media_tot <- mean(ecobici_semana$Tiempo_Viaje)

maX_int1 <- max(intervalo1$Tiempo_Viaje)
max_int2 <- max(intervalo2$Tiempo_Viaje)
max_int3 <- max(intervalo3$Tiempo_Viaje)
max_tot <- max(ecobici_semana$Tiempo_Viaje)

var_int1 <- var(intervalo1$Tiempo_Viaje)
var_int2 <- var(intervalo2$Tiempo_Viaje)
var_int3 <- var(intervalo3$Tiempo_Viaje)
var_tot <- var(ecobici_semana$Tiempo_Viaje)
